            ...
            {users.map((user) => (
              <tr key={user.id}>
                ...
                <td className="tableActions">
                  <button className="ghostButton" onClick={() => void patchUser(user.id, { role: user.role === 'admin' ? 'user' : 'admin' })}>
                    <Shield size={14} />
                    {user.role === 'admin' ? '降级' : '设管理员'}
                  </button>
                  <button className="dangerButton" onClick={() => void patchUser(user.id, { status: user.status === 'disabled' ? 'active' : 'disabled' })}>
                    <UserX size={14} />
